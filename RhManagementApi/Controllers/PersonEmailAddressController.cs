...
namespace RhManagementApi.Controllers
{
    ...
    public class PersonEmailAddressController : ControllerBase
    {
        ...
        public async Task<IActionResult> Get (int id)
        {
            ...
            var emailAddressDTO = this.mapper.Map<PersonEmailAdressDTO>(emailAddress);

            return Ok(emailAddressDTO);
        }

        [HttpPost]
        public async Task<IActionResult> Create(PersonEmailAdressDTO emailAddressDTO)
        {
            var emailAddress = this.mapper.Map<PersonEmailAdress>(emailAddressDTO);
            this.db.EmailAddresses.Add(emailAddress);
            await this.db.SaveChangesAsync();

            var readEmailAddressDTO = this.mapper.Map<PersonEmailAdressDTO>(emailAddress);
            return CreatedAtAction(nameof(Get),new {Id = emailAddress.BusinessEntityID}, readEmailAddressDTO);
        }

        [HttpPatch("{id}")]
        public async Task<IActionResult> Patch(int id, PersonEmailAdressDTO emailAddressDTO)
        {
            ...
            await this.db.SaveChangesAsync();
            return Ok(this.mapper.Map<PersonEmailAdressDTO>(emailAddress));
        }
    }
}